...
$role           = 'claimant';
// Auto-activate when the email is on the HR employee register (#1); otherwise
// the account stays disabled until an administrator activates it.
...
if ($ok) {
    ...
    log_audit($conn, 'user.register', 'user', $user_id,
        'claimant self-registration' . ($is_hr_employee ? ' (auto-activated via HR register)' : ''));
    // Notify the registrant (queued + sent if SMTP is configured; never fatal).
