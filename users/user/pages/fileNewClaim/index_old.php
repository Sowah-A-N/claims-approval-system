            ...
            <div class="content-wrapper">
                <form class="forms-sample" method="POST" name="newClaimForm" id="newClaimForm">
                    ...
                    <legend>Claim Information</legend>  
                        <div id="claimDataRow" class="container mt-4"></div>
                        <?php
					...
					<div class="form-group row mt-4">
						...
						<div class="col-sm-9">
							<input type="text" name="grandTotal" class="form-control" id="grandTotal" style="width:75%" readonly>
						</div>
					</div>
...
document.getElementById("addNewRow").addEventListener("click", function() {
    ...
    inputContainer.className = "col-12 col-md-8 d-flex align-items-center"; // Flex container for inputs
    ...
    timePicker1.addEventListener('input', function() {
        checkDuplicateDateTime(); // Ensure this function is defined elsewhere
        calculatePeriod(); // Ensure this function is defined elsewhere
		calculatePeriodTotalAmount();
    });
    ...
    timePicker2.addEventListener('input', function() {
		calculatePeriod();
		calculatePeriodTotalAmount();
    ...
    var textInput = document.createElement("input");
    textInput.setAttribute("type", "text");
    textInput.setAttribute("name", "subTotal[]");
    textInput.setAttribute("readonly", true);
    textInput.className = "form-control me-2"; // Bootstrap form-control and margin-end
    inputContainer.appendChild(textInput); // Append text input to the input container
	
	<?php if($fuelComponent): ?>
	var fuelComponentInput = document.createElement("input");
	fuelComponentInput.setAttribute("type", "checkbox"); // Set the input type to checkbox
	fuelComponentInput.setAttribute("name", "fuelComponent[]"); // Set the name attribute for the checkbox
	fuelComponentInput.className = "form-check-input me-2"; // Use Bootstrap's form-check-input class for styling
	// Create a label for the checkbox
	var fuelComponentLabel = document.createElement("label");
	fuelComponentLabel.className = "form-check-label"; // Bootstrap class for label styling
	fuelComponentLabel.textContent = "Fuel Component"; // Label text
	fuelComponentLabel.appendChild(fuelComponentInput); // Append the checkbox to the label

	inputContainer.appendChild(fuelComponentLabel); // Append the label (with checkbox) to the input container
	<?php endif; ?>
    ...
    deleteButton.className = "btn btn-danger ms-2"; // Use margin-start for spacing
    deleteButton.addEventListener('click', function() {
        newDiv.remove(); // Remove the current row
    });
    inputContainer.appendChild(deleteButton); // Append the delete button to the input container
    ...
    const fuelRate = <?php echo isset($fuelValue) ? $fuelValue : 0; ?>;//Fuel value goes here

	function calculatePeriodTotalAmount() {
		// Get the current rate from the rate input
		var rateInput = document.getElementById("rate");
		var rate = parseFloat(rateInput.value.replace('GH₵ ', '').replace(',', '')) || 0; // Handle NaN

		// Get all the period inputs
		var periodInputs = document.getElementsByName("period[]");
		var totalInputs = document.getElementsByName("subTotal[]");

		for (var i = 0; i < periodInputs.length; i++) {
			var periodValue = parseFloat(periodInputs[i].value) || 0; // Parse the period value or default to 0
			var totalValue = rate * periodValue; // Calculate total
			totalInputs[i].value = totalValue.toFixed(2); // Update total input, formatted to 2 decimal places
		}

		calculateGrandTotal(); // Call the grand total calculation after updating subtotals
	}

	function calculateGrandTotal() {
		var totalInputs = document.getElementsByName("subTotal[]");
		var fuelComponentInputs = document.getElementsByName("fuelComponent[]"); // Get all fuel component checkboxes
		var grandTotal = 0; // Initialize grand total

		// Sum all subtotals
		for (var i = 0; i < totalInputs.length; i++) {
			var subtotalValue = parseFloat(totalInputs[i].value) || 0; // Handle NaN
			grandTotal += subtotalValue; // Add subtotal to grand total
		}

		// Calculate the number of checked fuel components
		var checkedFuelComponents = Array.from(fuelComponentInputs).filter(cb => cb.checked).length;
		console.log(checkedFuelComponents);
		var fuelTotal = checkedFuelComponents * fuelRate; // Calculate fuel total
		console.log(checkedFuelComponents * fuelRate);
		grandTotal += fuelTotal; // Add fuel total to grand total

		// Update the grand total input
		var grandTotalInput = document.getElementById("grandTotal");
		grandTotalInput.value = "GH₵ " + grandTotal.toFixed(2); // Format grand total to 2 decimal places
	}

	// Event listeners for period inputs
	var periodInputs = document.getElementsByName("period[]");
	periodInputs.forEach(function(periodInput) {
		periodInput.addEventListener('input', calculatePeriodTotalAmount);
	});

	// Event listeners for fuel component checkboxes
	var fuelComponentInputs = document.getElementsByName("fuelComponent[]");
	fuelComponentInputs.forEach(function(fuelInput) {
		fuelInput.addEventListener('change', calculateGrandTotal);
	});
