                ...
                <form class="forms-sample" method="POST" name="newClaimForm" id="newClaimForm">
                    ...
                    <fieldset>
                        ...
                        <div class="form-group row">
                            <label class="col-sm-3 col-form-label" for="user">Full Name</label>
                            <div class="col-sm-9">
                                <input type="text" name="user" class="form-control" id="user" style="width:50%" value="<?php outputFullName(); ?>" readonly>
                            </div>
                        </div>
                    ...
                    <fieldset>
                        ...
                        <?php
                        if($savedClaimDataResult != ""):
                            if ($savedClaimDataResult->num_rows > 0):
...
function addNewRow(date = '', startTime = '', endTime = '', period = '') {
    // Create a new div element
    var newDiv = document.createElement("div");
    newDiv.className = "added-div row mb-2"; // Add Bootstrap classes for styling

    // Create date picker input
    var datePicker = document.createElement("input");
    datePicker.setAttribute("type", "date");
    datePicker.setAttribute("name", "date[]");
    datePicker.className = "form-control col";
    datePicker.value = date;
    newDiv.appendChild(datePicker);

    // Create first time picker input
    var timePicker1 = document.createElement("input");
    timePicker1.setAttribute("type", "time");
    timePicker1.setAttribute("name", "startTime[]");
    timePicker1.className = "form-control col";
    timePicker1.value = startTime;
    newDiv.appendChild(timePicker1);

    // Create second time picker input
    var timePicker2 = document.createElement("input");
    timePicker2.setAttribute("type", "time");
    timePicker2.setAttribute("name", "endTime[]");
    timePicker2.className = "form-control col";
    timePicker2.value = endTime;
    newDiv.appendChild(timePicker2);

    // Create text input for period
    var textInput = document.createElement("input");
    textInput.setAttribute("type", "text");
    textInput.setAttribute("name", "period[]");
    textInput.setAttribute("readonly", true);
    textInput.className = "form-control col";
    textInput.value = period;
    ...
    timePicker2.addEventListener('input', calculatePeriod);

    // Run the duplicate check and period calculation after adding the row
    checkDuplicateDateTime();
    calculatePeriod();
}

// Attach event listeners to existing rows loaded from the server
function attachEventListenersToExistingRows() {
    ...
    endTimeInputs.forEach(function(input) {
        input.addEventListener('input', calculatePeriod);
    });
}

// Call this function after the page loads or after rows are dynamically added from the database
...
// Function to calculate the period between start and end times
function calculatePeriod() {
    // Implement the logic to calculate and update the period here
}
    ...
    // Function to reset the form #1292 - Truncated incorrect DOUBLE value: 'ICT'
    function resetForm(){
