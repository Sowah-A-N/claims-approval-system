<?php
    ...
    $stmt = $conn->prepare("INSERT INTO claim_details (userId, department, programme, course) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("isss", $userId, $department, $programme, $course);
    $result = $stmt->execute();

    // Check for errors in claim details insertion
    if ($result === false) {
        echo json_encode(array("error" => "Error submitting claim: " . $conn->error));
        exit;
    } else {
        echo json_encode(array("success" => "Claim submitted successfully!"));
    }
    ...
    if (isset($date)) {
        for ($i = 0; $i < count($date); $i++) {
            ...
            if ($result === false) {
                echo json_encode(array("error" => "Error submitting claim data: " . $conn->error));
                exit;
            } else {
                echo json_encode(array("success" => "Claim data submitted successfully!"));
            }
        }
    }
