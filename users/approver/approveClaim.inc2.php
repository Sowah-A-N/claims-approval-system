...
try {
    ...
    // Check if we need to mark the claim as complete
    if ($newStage > 5) {
        ...
        $stmt->bind_param('i', $claimId);
        if (!$stmt->execute() || $stmt->affected_rows <= 0) {
            throw new Exception('Failed to mark the claim as complete.');
        }
        $stmt->close();
		
		 // Insert the completed stage into the approval stages
        $stmt = $conn->prepare(
        ...
        $status = 'Completed'; // Assuming at this point, stage is 'Completed'
        $stmt->bind_param('iis', $claimId, $newStage, $status);
        $stmt->execute();

        if ($stmt->affected_rows <= 0) {
            throw new Exception('Failed to insert completed stage.');
        }
